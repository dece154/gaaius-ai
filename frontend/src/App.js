...
import { 
  ...
  FileCode, FolderOpen, Hammer, Eye, Code, Settings, CreditCard
} from "lucide-react";
...
const AdBanner = ({ onUpgrade }) => {
  ...
  return (
    <div className="fixed bottom-20 left-0 right-0 md:left-72 p-4 z-40">
      <div className="max-w-4xl mx-auto glass rounded-xl p-3 flex items-center justify-between border border-yellow-500/30">
      ...
      <div className={`max-w-[80%] ${isUser ? "bg-primary/20 border-primary/30 rounded-br-sm" : "bg-secondary/50 border-white/5 rounded-bl-sm"} border rounded-2xl p-4`}>
        {message.model_used && (
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-xs text-muted-foreground uppercase">{message.model_used}</span>
          </div>
        )}
...
const GenerationResult = ({ data, type }) => {
  ...
  const colors = { image: "cyan", video: "orange", audio: "green", file: "pink" };
    ...
    <div className="glass rounded-2xl overflow-hidden" data-testid={`result-${data.id}`}>
      ...
      <div className="p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={`font-mono text-xs text-${color}-400 uppercase`}>{data.model_used}</span>
        </div>
        ...
        <a href={url} download target="_blank" rel="noopener noreferrer" className={`mt-3 inline-flex items-center gap-2 text-xs text-${color}-400`}>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {projects.map(project => (
          <div key={project.id} className="glass rounded-xl p-4 hover:border-primary/50 border border-white/10 cursor-pointer">
    ...
    try {
      if (mode === "chat") {
        ...
        setMessages(prev => {
          ...
          return [...filtered, { ...tempUserMsg, id: `user-${Date.now()}` }, { id: res.data.id, role: "assistant", content: res.data.content, model_used: res.data.model_used, timestamp: res.data.timestamp }];
        });
      } else if (mode === "image") {
        ...
        <aside className={`fixed md:relative z-50 h-full w-72 glass border-r border-white/10 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`} data-testid="sidebar">
          ...
          <div className="p-6 border-b border-white/10">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" variant="ghost" onClick={createSession} className="h-8 w-8 p-0" data-testid="new-chat-btn">
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            <div className="p-4"><p className="font-mono text-xs text-muted-foreground uppercase">Chats</p></div>
            <ScrollArea className="flex-1 px-4">
              {sessions.map(session => (
                <div key={session.id} className={`group flex items-center gap-3 p-3 rounded-xl mb-2 cursor-pointer transition-all ${currentSession?.id === session.id ? "bg-primary/20 border border-primary/30" : "hover:bg-white/5"}`}
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteSession(session.id); }} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-white/10 rounded" data-testid={`delete-session-${session.id}`}>
            ...
            {user ? (
              <div className="glass-light rounded-xl p-3">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <button onClick={logout} className="p-1 hover:bg-white/10 rounded"><LogOut className="w-4 h-4 text-muted-foreground" /></button>
                </div>
                {!user.is_pro && (
                  <Button size="sm" onClick={() => setShowPro(true)} className="w-full bg-yellow-500 hover:bg-yellow-600 text-black">
                    <Crown className="w-3 h-3 mr-1" /> Go Pro - $1
                  </Button>
                )}
              </div>
            ) : (
              <Button onClick={() => setShowAuth(true)} className="w-full" variant="outline">
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <div className="absolute bottom-0 left-0 right-0 md:left-72 p-4 md:p-6">
            <form onSubmit={handleSubmit} className="max-w-4xl mx-auto">
              <div className={`glass rounded-2xl p-2 flex items-center gap-2 ${ModeConfig.borderColor} border`}>
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/build" element={<div className="h-screen bg-[#050505]"><BuildPage /></div>} />
      </Routes>
    </BrowserRouter>
  );
}
